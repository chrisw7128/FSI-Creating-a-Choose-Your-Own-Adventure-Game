...
if (firstAnswer == "left") {
  let secondAnswer = window.prompt(
    `You come across a stray cat. It scampers off down a small hole, just large enough for you to crawl through. Do you follow it, or continue on your path? (follow or stay)`
  );
  if (secondAnswer == "follow") {
    let thirdAnswer = window.prompt(
      "You follow the cat to a colony of cats, nestled in a fort of warm blankets and subsisting off of inexplicably warm soup. They are content with you staying, but you wonder if you should alert the world to this magical safe haven. (stay or spread the word)"
    );
    if (thirdAnswer == "stay") {
  ...
  if (secondAnswer == "past the dragon") {
    let thirdAnswer = window.prompt(
      "The dragon wakes up and sits upright. You only have a moment to respond, to stay or run: got the treasure. (stay or run)"
    );
    if ((thirdAnswer = "stay")) {
      let fourthAnswer = window.alert(
